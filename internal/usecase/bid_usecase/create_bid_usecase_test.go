...
func (f *fakeBidRepo) CreateBid(ctx context.Context, bids []bid_entity.Bid) *internal_error.InternalError {
	f.mu.Lock()
	defer f.mu.Unlock()
	batch := make([]bid_entity.Bid, len(bids))
	copy(batch, bids)
...
func (f *fakeBidRepo) batches() [][]bid_entity.Bid {
	f.mu.Lock()
	defer f.mu.Unlock()
	out := make([][]bid_entity.Bid, len(f.createdBatches))
...
func TestCreateBid_FlushesOnMaxBatchSize(t *testing.T) {
	...
	require.Nil(t, uc.CreateBid(ctx, bid_usecase.BidInputDTO{
...
func TestCreateBid_InvalidUserId_ReturnsBadRequest(t *testing.T) {
	...
	err := uc.CreateBid(context.Background(), bid_usecase.BidInputDTO{
